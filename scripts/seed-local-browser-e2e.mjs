...
const profileRows = [
  `(${sqlText(fixtures.admin.id)}, ${sqlText(fixtures.admin.email)}, ${sqlText(fixtures.admin.name)}, ${sqlText(fixtures.admin.phone)}, 'admin', 15000, true, date '2024-01-15')`,
  ...fixtures.workers.map(
    (worker, index) =>
      `(${sqlText(worker.id)}, ${sqlText(worker.email)}, ${sqlText(worker.name)}, ${sqlText(worker.phone)}, 'worker', ${10000 + index * 100}, true, date '2024-01-15')`,
  ),
]
...
insert into public.profiles
  (id, email, name, phone, role, hourly_wage, is_active, hired_at)
