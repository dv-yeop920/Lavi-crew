...
- 화면 경로: 알바 화면은 `/`, 관리자 화면은 `/admin`을 기준으로 한다.
...
- 여러 테이블을 함께 변경하는 마감·출석 확정·급여 확정은 Postgres RPC로 원자 처리한다.
...
- 알바는 자신의 프로필·신청·배정·출석·급여만 접근하도록 정책을 작성한다.
...
- 출석 확정 이후의 변경은 일반 수정이 아니라 관리자 정정 흐름으로 처리한다.

## 9. 변경하면 안 되는 업무 규칙

- 급여는 관리자가 확정한 실제 출석 시간만 계산한다.
- 하루 9시간까지 기본 시급, 9시간 초과분은 해당 포지션 시급의 1.5배로 계산한다.
- 식대, 교통비, 주휴수당은 MVP 급여 계산에서 제외한다.
- 스케줄 신청은 관리자가 마감하기 전까지만 취소할 수 있다.
- 관리자는 미신청자 배정, 긴급 대체 배정, 교육 여부 표시를 할 수 있다.
...
- 확정 출석·급여·배정 이력을 `DELETE`로 제거하지 않는다.
