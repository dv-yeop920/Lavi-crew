...
export async function getAdminDashboardRecords(input: {
  ...
  const [weekSchedules, period, monthSchedules] = await Promise.all([
    supabase
      .from('shifts')
    ...
    supabase
      ...
      .eq('year_month', input.monthStart)
      .maybeSingle(),
    supabase
      .from('shifts')
      .select('work_date')
      ...
      .lt('work_date', input.monthEndExclusive),
  ])
  if (weekSchedules.error || period.error || monthSchedules.error)
