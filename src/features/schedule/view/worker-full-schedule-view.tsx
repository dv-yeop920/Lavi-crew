...
import { formatDateLabel, monthLabel } from '@/features/schedule/model/date-format'
import type { ScheduleHistoryRow } from '@/features/schedule/schema/schedule-history-view-model'
...
export function WorkerFullScheduleView({
  month,
  rows,
}: {
  month: string
  ...
  return (
    <div className={layout.page}>
      <PageHeader
        backHref="/schedule"
        backLabel="내 스케줄로 돌아가기"
        eyebrow="확정 일정"
        title={`${monthLabel(month)} 전체 일정`}
        description="해당 월에 등록된 전체 포지션과 배정을 확인합니다."
      />
      {currentRow ? (
        <>
