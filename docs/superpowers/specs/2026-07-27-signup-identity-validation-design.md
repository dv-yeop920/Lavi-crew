...
- 사용자는 회원가입 화면에서 이름, 이메일, 휴대폰 번호, 비밀번호, 라비에벨 전용 코드와 카카오
  알림톡 동의를 제출한다.
...
이번 변경은 이메일 고유성 정책, 관리자 초대 코드 관리 UI, OAuth 온보딩 정책을 변경하지 않는다.
동일한 프로필·초대 코드 규칙을 사용하는 OAuth 온보딩은 서버 검증 결과와 오류 메시지 계약만
재사용할 수 있도록 경계를 유지한다.
