...
Web Push 실발송은 이번 범위에서 제외한다. 대신 푸시 구독이 있는 배정자마다
...
- 이후 Web Push Adapter는 pending 로그를 처리하고 성공·실패 상태를 갱신하는 별도 작업으로 추가한다.
...
- 사용자 흐름의 `알림 발송`은 이번 범위에서 `알림 발송 대기 로그 생성`으로 수정한다.
...
- Web Push 실발송과 재시도 worker의 provider 결과 갱신
- 기존 일별 일정의 수정·취소 구현
