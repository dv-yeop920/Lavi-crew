...
- 월별 일정 등록은 신청 기간이 열려 있는 동안에도 허용하며 마감일과 기간 상태를 변경하지 않는다. 등록 시점까지 해당 날짜에 신청 상태인 인원만 배정할 수 있다. 신청 기간 생성·마감·재개는 별도 RPC로 처리하고, 일정 생성 이력이 있는 기간은 마감일 변경과 재개를 모두 거부한다.
