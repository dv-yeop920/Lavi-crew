-- Remote migration version: 20260730121911.
alter table public.shifts
  add column cancelled_at timestamptz,
  add column cancelled_by uuid references public.profiles(id) on delete restrict,
  add column cancellation_reason text,
  add constraint shifts_cancellation_audit_check check (
    (status <> 'cancelled' and cancelled_at is null and cancelled_by is null and cancellation_reason is null)
    or (
      status = 'cancelled'
      and cancelled_at is not null
      and cancelled_by is not null
      and char_length(trim(cancellation_reason)) between 2 and 500
    )
  );

alter table public.shift_assignments
  add column cancelled_by uuid references public.profiles(id) on delete restrict,
  add constraint shift_assignments_cancellation_actor_check check (
...
create index shift_assignments_cancelled_by_idx on public.shift_assignments (cancelled_by)
  where cancelled_by is not null;

create table private.daily_schedule_mutation_requests (
  request_id uuid primary key,
  requested_by uuid not null references public.profiles(id) on delete restrict,
  operation text not null check (operation in ('update_schedule', 'cancel_schedule', 'confirm_attendance')),
...
alter table private.daily_schedule_mutation_requests enable row level security;
revoke all on private.daily_schedule_mutation_requests from public, anon, authenticated;
create index daily_schedule_mutation_requests_actor_idx
  on private.daily_schedule_mutation_requests (requested_by, created_at desc);

create function public.update_daily_schedule(
  p_request_id uuid,
  p_shift_id uuid,
...
create function public.cancel_daily_schedule(
  p_request_id uuid,
  p_shift_id uuid,
  p_expected_shift_updated_at timestamptz,
  p_reason text
...
alter function public.confirm_attendance_and_payroll(
  uuid, public.attendance_status, timestamptz, timestamptz, text
) set schema private;

create function public.confirm_attendance_and_payroll(
  p_request_id uuid,
  p_record_id uuid,
...
revoke all on function private.confirm_attendance_and_payroll(
  uuid, public.attendance_status, timestamptz, timestamptz, text
) from public, anon, authenticated;
revoke all on function public.update_daily_schedule(uuid, uuid, timestamptz, smallint, time, time, jsonb)
  from public, anon;
revoke all on function public.cancel_daily_schedule(uuid, uuid, timestamptz, text)
  from public, anon;
...
grant execute on function public.update_daily_schedule(uuid, uuid, timestamptz, smallint, time, time, jsonb)
  to authenticated;
grant execute on function public.cancel_daily_schedule(uuid, uuid, timestamptz, text)
  to authenticated;
