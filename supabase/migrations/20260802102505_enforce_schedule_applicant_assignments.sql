-- Enforce date-based schedule applications at the privileged RPC boundary.
-- Monthly publishing only creates assignments, so every worker must have applied.
-- Daily editing preserves already-confirmed worker IDs, while newly introduced workers must apply.
